...
class TaskManager
  ...
  def display_high_priority_tasks
    high_priority_tasks = tasks.select do |task| # SOLUTION: we initialized local variable tasks
      # that was local to the display_high_priority_tasks method. So when we call select on tasks,
      # Ruby attempts to invoke select on the local variable tasks, which has no assignment yet and
      # therefore has a value of nil. The local variable tasks shadows the tasks getter method even though
      # we have not assigned anything to it yet.
      task.priority == :high
    end
